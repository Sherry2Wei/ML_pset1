...
factors = all_factor.columns[1:]
factor_taken = []
for item in result:
    fac = [factors[item[-1].tolist().index(x)] for x in item[-1].tolist() if x>0]
    factor_taken = factor_taken + fac
factor_taken

from collections import Counter

freq = Counter(factor_taken)
freq_serie = pd.Series(dict(Counter(sorted(factor_taken))))
freq_df = pd.DataFrame(freq_serie,columns = ['freq'])
